...
shinyServer(function(session, input, output) {
  ...
  observeEvent(input$link_to_history_tab, {
    updateNavbarPage(session, inputId = "menu", selected = "Evolution of metrics")
  })
  observeEvent(input$link_to_compare_tab, {
    updateNavbarPage(session, inputId = "menu", selected = "Compare metric by country")
  })
  observeEvent(input$link_to_analysis_tab, {
    updateNavbarPage(session, inputId = "menu", selected = "Analyses")
  })
  observeEvent(input$link_to_biomimicry_tab, {
    updateNavbarPage(session, inputId = "menu", selected = "Biomimicry and Life's Principles")
  })
  # observeEvent(input$link_to_history_tab, {
  #   updateNavbarPage(session, inputId = "menu", selected = "Evolution of metrics")
  # })
...
### WORLD TREND PLOTS TO QUESTION
  
  trend_x <- list(title = "Year",
                  autotick = FALSE,
                  ticks = "outside",
                  range = c(1960, 2020),
                  dtick = 10,
  ...
  output$econ_view <- renderPlotly({
    
    plot_ly(trend_data, x = ~year, y = ~med_gdp, name = "Median GDP",
            text = paste0("Range: "), type = "scatter", mode = "lines+markers") %>%
      # add_trace(y = ~min_gdp, name = "Minimum GDP", mode = "lines+markers") %>%
      # add_trace(y = ~max_gdp, name = "Maximum GDP", mode = "lines+markers") %>%
      add_trace(y = ~avg_gdp, name = "Average GDP", mode = "lines+markers") %>%
      layout(legend = list(x = 0.05, y = 1), xaxis = trend_x, yaxis = list(title = ""))
    
  })
  
  output$equal_view <- renderPlotly({
    
    plot_ly(trend_data, x = ~year, y = ~med_gini, name = "Median GINI",
            type = "scatter", mode = "lines+markers") %>%
      # add_trace(y = ~min_gini, name = "Minimum GINI", mode = "lines+markers") %>%
      # add_trace(y = ~max_gini, name = "Maximum GINI", mode = "lines+markers") %>%
      add_trace(y = ~avg_gini, name = "Average GINI", mode = "lines+markers")
    
  })
  
  output$social_view <- renderPlotly({
    
    plot_ly(trend_data, x = ~year, y = ~med_hdi, name = "Median HDI",
            type = "scatter", mode = "lines+markers") %>%
      # add_trace(y = ~min_hdi, name = "Minimum HDI", mode = "lines+markers") %>%
      # add_trace(y = ~max_hdi, name = "Maximum HDI", mode = "lines+markers") %>%
      add_trace(y = ~avg_hdi, name = "Average HDI", mode = "lines+markers")
    
  })
  
  output$eco_view <- renderPlotly({
    
    plot_ly(trend_data, type = "scatter", x = ~year, y = ~med_econs, name = "Median Consumption", mode = "lines+markers", line = list(color = "red"), marker = list(color = "red")) %>%
      add_trace(y = ~med_ecap, name = "Median Biological Capacity", mode = "lines+markers", line = list(color = "green"), marker = list(color = "green")) %>%
      # add_trace(y = ~med_fp, name = "Median Reserve (+) / deficit (-)", mode = "lines+markers", fill = "tozeroy") %>%
      layout(legend = list(x = 0.05, y = 1), xaxis = trend_x, yaxis = list(title = ""))
    
  })
  
  output$happy_view <- renderPlotly({
    
    plot_ly(trend_data, x = ~year, y = ~med_hap, name = "Median Happiness",
            type = "scatter", mode = "lines+markers") %>%
      # add_trace(y = ~min_hap, name = "Minimum Happiness", mode = "lines+markers") %>%
      # add_trace(y = ~max_hap, name = "Maximum Happiness", mode = "lines+markers") %>%
      add_trace(y = ~avg_hap, name = "Average Happiness", mode = "lines+markers")
    
  })
  
### COMMON METRICS AXES FOR COUNTRY DETAILS
